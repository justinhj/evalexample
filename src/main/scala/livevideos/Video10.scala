...
object Scala3EvalEither extends App:
  ...
  def eval[A : Numeric](exp: Exp[A]): WithEnv[A] =
    exp match
      ...
      case Val(value) => WriterT.lift[EvalResult,List[String],A](Right(value))
  ...
  def handleVar[A](s: String): WithEnv[A] =
    summonEnv.get(s) match {
      case Some(value) => WriterT.lift[EvalResult,List[String],A](Right(value))
      case None => WriterT.lift[EvalResult,List[String],A](Left(EvalError.SymbolNotFound))
    }
